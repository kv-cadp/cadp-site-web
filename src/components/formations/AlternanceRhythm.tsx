import SectionTitle from "@/components/ui/SectionTitle";
import type { AlternanceRhythm as AlternanceRhythmType } from "@/types/formation";

interface AlternanceRhythmProps {
  rhythm: AlternanceRhythmType;
}

const days = ["Lun", "Mar", "Mer", "Jeu", "Ven"];

export default function AlternanceRhythm({ rhythm }: AlternanceRhythmProps) {
  const hasAlternating = JSON.stringify(rhythm.weekLong) !== JSON.stringify(rhythm.weekShort);

  return (
    <section className="py-20 bg-cream">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <SectionTitle subtitle="Un rythme pensé pour apprendre et travailler en parallèle.">
          Rythme de l&apos;alternance
        </SectionTitle>

        {hasAlternating ? (
          /* Deux semaines types côte à côte */
          <div className="bg-white rounded-xl shadow-sm p-6 md:p-8 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Semaine A — 3 jours campus */}
              <div>
                <p className="text-sm font-semibold text-gold mb-3 text-center">
                  Semaine A — {rhythm.weekLong.length} jours campus
                </p>
                <div className="grid grid-cols-5 gap-2">
                  {days.map((day, i) => {
                    const isSchool = rhythm.weekLong.includes(i);
                    return (
                      <div
                        key={`long-${day}`}
                        className={`rounded-lg p-3 text-center transition-all ${
                          isSchool
                            ? "bg-navy-deep text-white"
                            : "bg-gold/10 text-gold border border-gold/20"
                        }`}
                      >
                        <span className="block text-xs font-semibold uppercase mb-0.5 opacity-70">
                          {day}
                        </span>
                        <span className="block text-[10px] font-bold">
                          {isSchool ? "Campus" : "Entreprise"}
                        </span>
                      </div>
                    );
                  })}
                </div>
              </div>

              {/* Semaine B — 2 jours campus */}
              <div>
                <p className="text-sm font-semibold text-gold mb-3 text-center">
                  Semaine B — {rhythm.weekShort.length} jours campus
                </p>
                <div className="grid grid-cols-5 gap-2">
                  {days.map((day, i) => {
                    const isSchool = rhythm.weekShort.includes(i);
                    return (
                      <div
                        key={`short-${day}`}
                        className={`rounded-lg p-3 text-center transition-all ${
                          isSchool
                            ? "bg-navy-deep text-white"
                            : "bg-gold/10 text-gold border border-gold/20"
                        }`}
                      >
                        <span className="block text-xs font-semibold uppercase mb-0.5 opacity-70">
                          {day}
                        </span>
                        <span className="block text-[10px] font-bold">
                          {isSchool ? "Campus" : "Entreprise"}
                        </span>
                      </div>
                    );
                  })}
                </div>
              </div>
            </div>

            <div className="flex flex-wrap justify-center gap-6 text-sm mt-6">
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 rounded bg-navy-deep" />
                <span className="text-gray-mid">Campus</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 rounded bg-gold/20 border border-gold/30" />
                <span className="text-gray-mid">Entreprise</span>
              </div>
            </div>
          </div>
        ) : (
          /* Rythme fixe (ADVF) — une seule semaine */
          <div className="bg-white rounded-xl shadow-sm p-6 md:p-8 mb-8">
            <div className="grid grid-cols-5 gap-3 mb-6">
              {days.map((day, i) => {
                const isSchool = rhythm.weekLong.includes(i);
                return (
                  <div
                    key={day}
                    className={`rounded-lg p-4 text-center transition-all ${
                      isSchool
                        ? "bg-navy-deep text-white"
                        : "bg-gold/10 text-gold border border-gold/20"
                    }`}
                  >
                    <span className="block text-xs font-semibold uppercase mb-1 opacity-70">
                      {day}
                    </span>
                    <span className="block text-sm font-bold">
                      {isSchool ? "Campus" : "Entreprise"}
                    </span>
                  </div>
                );
              })}
            </div>

            <div className="flex flex-wrap justify-center gap-6 text-sm">
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 rounded bg-navy-deep" />
                <span className="text-gray-mid">{rhythm.schoolDays} jours au campus</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 rounded bg-gold/20 border border-gold/30" />
                <span className="text-gray-mid">{rhythm.companyDays} jours en entreprise</span>
              </div>
            </div>
          </div>
        )}

        <p className="text-gray-mid text-center max-w-2xl mx-auto leading-relaxed">
          {rhythm.description}
        </p>
      </div>
    </section>
  );
}
